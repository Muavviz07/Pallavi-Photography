...
export default function RecognitionsAndAwardsPage() {
  ...
  const { t, lang } = useTranslation("recognitions");
  ...
  const displayTitle = t("title", "RECOGNITIONS & AWARDS");
  const displayDesc = t("desc", "Grateful for the moments, and honored by the recognition — here are a few milestones that have shaped my journey.");
    ...
    <>
      ...
      <BreadcrumbsBanner
        ...
        paths={[
          ...
          { label: lang === "FR" ? "Distinctions" : "Recognitions & Awards" }
        ]}
      />
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              ...
              <p className="text-xs uppercase tracking-wider text-[#6E635F] font-light">{t("loading", "Loading recognitions...")}</p>
            </div>
          ) : awards.length === 0 ? (
            <div className="text-center py-20 bg-[#FAF8F5] border border-[#DCD0C0]/20 rounded-sm">
              <p className="text-sm font-light text-[#6E635F]">{t("noAwards", "No recognitions or awards published yet.")}</p>
            </div>
          ) : (
            ...
            <div className="columns-1 sm:columns-2 md:columns-3 gap-6 [column-fill:_balance] box-border">
              {awards.map((award, idx) => {
                const awardTitle = t(`award_title_${award.id}`, award.title);
                return (
                  <div
                    key={award.id}
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#2C2623]/75 via-[#2C2623]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                      <span className="text-[9px] uppercase tracking-[0.25em] text-[#C4A484] font-semibold block mb-1 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500 delay-75">
                        {lang === "FR" ? "Voir l'Award" : "View Award"}
                      </span>
      ...
      {activeImageIndex !== null && awards.length > 0 && (() => {
        const currentAward = awards[activeImageIndex];
        const awardTitle = t(`award_title_${currentAward.id}`, currentAward.title);
        const awardDesc = t(`award_desc_${currentAward.id}`, currentAward.description || "");
